...
for t = 1:T
    ...
    %    C_r = [C_r; 0<=P_r(:,t)<=0];%测试的时候才用。
     ...
     st1 = [st1; 0<=beta<=0.7]; %新能源出力小于预测的波动出力
...
%% 绘制新能源渗透率与目标函数解析关系
plp = Opt (st1, obj, beta, P_s); % 利用MPT3工具包建模
solution = plp.solve; % 利用MPT3工具包求解多参数规划，结果存在solution里面

figure;
solution.xopt.fplot('obj','linewidth',5);

% for i = 1:N_s
%     figure;
%     solution.xopt.fplot('primal', 'position', i,'linewidth',5);
%     xlabel('beta');
